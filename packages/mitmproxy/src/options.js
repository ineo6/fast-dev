import interceptors from "./lib/interceptor";
import dnsUtil from "./lib/dns";
import log from "./utils/log";
import matchUtil from "./utils/match";
import headerTagMiddleware from "./lib/proxy/middleware/headerTagMiddleware";
...
export default config => {
  ...
  const forward = serverConfig.forward;

  const middlewares = [];

  middlewares.push(headerTagMiddleware);
  ...
  const options = {
    ...
    sslConnectInterceptor: (req, cltSocket, head) => {
      ...
      const inWhiteList = matchUtil.matchHostname(whiteList, hostname);
      if (inWhiteList) {
        log.info("白名单域名，不拦截", hostname, whiteList);
        return false; // 所有都不拦截
      }
      // 配置了拦截的域名，将会被代理
